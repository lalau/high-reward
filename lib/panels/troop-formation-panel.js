...
function TroopFormationPanel(game, x, y) {
...
TroopFormationPanel.prototype.render = function(troop) {
    if (troop) {
        this._troop = troop;
    }
...
TroopFormationPanel.prototype._renderTitle = function() {
    ...
    if (this._nameText) {
        ...
        textUtil.renderText(this.game, 6, 7, 'TROOP FORMATION', { parent: this });
        this._nameText = textUtil.renderText(this.game, 70, 17, formationName, { parent: this, scale: 2, type: 'value' });
    }
};

TroopFormationPanel.prototype._renderFormation = function() {
    var game = this.game;
    var formationGraphic = this._formationGraphic;
    var troop = this._troop;
    var formationIndex = troop.formationIndex;
    var createSlot = false;

    if (!formationGraphic) {
        formationGraphic = new Phaser.Graphics(game, 0, 0);
        this.addChild(formationGraphic);
        this._formationGraphic = formationGraphic;
        createSlot = true;
    }

    formations[formationIndex].slots.forEach(createSlot ? this._renderSlot : this._updateSlot, this);
};

TroopFormationPanel.prototype._renderSlot = function(slot, slotIndex) {
    var troop = this._troop;
    var slotPosition = this._getSlotPosition(slot);
    var unitSlot = new UnitSlot(this.game, slotPosition.x, slotPosition.y, slotIndex);
    var memberIndex = troop.formation[slotIndex];

    unitSlot.render(troop.getUnitAt(memberIndex), memberIndex);
    this._formationGraphic.addChildAt(unitSlot, slotIndex);
    this._initUnitSlotInput(unitSlot);
};

TroopFormationPanel.prototype._updateSlot = function(slot, slotIndex) {
    var unitSlot = this._formationGraphic.getChildAt(slotIndex);
    unitSlot.position = this._getSlotPosition(slot);
};
...
TroopFormationPanel.prototype._handleUnitSlotMoveEnd = function(unitSlot) {
    ...
    unitSlot.render(troop.getUnitAt(movingMemberIndex), movingMemberIndex);
    this._movingSlot.render(troop.getUnitAt(targetMemberIndex), targetMemberIndex);

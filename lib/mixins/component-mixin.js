...
componentMixin._renderUnitImage = function(x, y, unit, key, parent) {
    ...
    frameName = unit.getAssetKey() + '-stand.png';

    if (image) {
        if (image.frameName === frameName) {
            return image;
        }
        image.destroy();
    }

    this[key] = new Phaser.Image(this.game, x, y, 'sprites', frameName);
    parent = parent || this;
...
componentMixin._updatePortrait = function(characterKey) {
    ...
    if (this._portrait.frameName !== frameName) {
        x = this._portrait.x;
        y = this._portrait.y;

...
Bulletin.prototype.create = function() {
    ...
    if (wip) {
        ...
        game.state.start(STATES.Conversation, undefined, undefined, {
            ...
            onEnd: function() {
                game.state.start(STATES.StationMenu);
            }
        });
...
Bulletin.prototype._renderPointer = function(posting) {
    var postingPoi = this._getPostingPoi(posting);
    this._pointer = this._screen.addChild(new Phaser.Image(this.game, postingPoi.x + 10, postingPoi.y - 40, 'icons', 'pointer.png'));
};

...
Unit.prototype.getExpert = function() {
    return Math.min(Math.floor(this.attrs.expertRaw / 100), 999);
};
...
Unit.prototype._computeAttrWithExpert = function(base) {
    return Math.round(base * Math.pow(1.5, this.getExpert() / 100));
};

Unit.prototype.updateExperts = function() {
    var typeConfig = this.getTypeConfig();
    var maxAttrs = typeConfig && typeConfig.max;
    ...
    var attrs = this.attrs;
    var baseAttrs = typeConfig.attributes;
    var newMaxHp = Math.min(this._computeAttrWithExpert(baseAttrs.hp), maxAttrs.hp);
    var hpDiff = newMaxHp - attrs.maxHp;
    var newShoot = Math.min(this._computeAttrWithExpert(baseAttrs.shoot), maxAttrs.shoot);
    var newDefence = Math.min(this._computeAttrWithExpert(baseAttrs.defence), maxAttrs.defence);
    ...
    if (newShoot > attrs.shoot) {
        attrs.shoot = newShoot;
    }

    if (newDefence > attrs.defence) {
        attrs.defence = newDefence;
    }
};

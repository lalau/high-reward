...
function MemberRow(game, x, y, member, index, config) {
    Phaser.Graphics.call(this, game, x, y);

    config = config || {};
    this._member = member;
    this._index = index;
    this._view = config.view || 'default';
...
MemberRow.prototype._renderUnitSlot = function() {
    ...
    if (this._view === 'battle') {
        this._unitSlot.enableSubSlot('item', UnitSlot.WIDTH / 2, UnitSlot.WIDTH - UnitSlot.SUBSLOT_WIDTH);
    }
...
MemberRow.prototype._renderTitle = function() {
    ...
    this._titleText = textUtil.renderText(this.game, MemberRow.X_PADDING[this._view], 3, title, { type: 'value', scale: 1.5 });
...
MemberRow.prototype._renderMembersAttributes = function() {
    ...
    this._renderAttributes(MemberRow.X_PADDING[this._view], 15, {
        key: 'member',
        pad: 3,
        parent: this,
        attrs: [{

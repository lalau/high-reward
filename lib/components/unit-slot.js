...
UnitSlot.prototype.enableSubSlot = function(type) {
    ...
    this._subSlot = this.addChild(new Phaser.Image(game, UnitSlot.SUBSLOT_X, UnitSlot.SUBSLOT_Y, 'icons', (imageKey || 'empty-item-slot') + '.png'));
    this._subSlotType = type;

    if (imageKey && this._subSlot && unit) {
        if (type === 'item') {
            this._subSlot.addChild(textUtil.renderText(game, 10, 15, 'X' + unit.items.length, {type: 'value', parent: this}));
        }
    }
};

...
EasyStar.js = function() {
    ...
    **/
    ...
    this.setSearchDeltas = function(_deltas) {
        deltas = _deltas;
    };
    ...
    this.calculate = function() {
        ...
        for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
            ...
            deltas.forEach(function(delta) {
                var xBound = (delta.x < 0 && searchNode.x >= -delta.x) || (delta.x >= 0 && searchNode.x < collisionGrid[0].length - delta.x);
                var yBound = (delta.y < 0 && searchNode.y >= -delta.y) || (delta.y >= 0 && searchNode.y < collisionGrid.length - delta.y);

                if (xBound && yBound) {
                    tilesToSearch.push({ instance: instances[0], searchNode: searchNode, x: delta.x, y: delta.y,
                        cost: STRAIGHT_COST * getTileCost(searchNode.x + delta.x, searchNode.y + delta.y) });
                }
            });

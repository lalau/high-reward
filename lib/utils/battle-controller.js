...
BattleController.prototype._handleItem = function() {
    var recoveringCandidates = [];

    this._troops.forEach(function(troop, troopIndex) {
        troop.members.forEach(function(unit, unitIndex) {
            var item = unit && !unit.battleAttrs.isDead && unit.items[0];
            if (item && item.type === 'recovery' && item.amount <= (unit.attrs.maxHp - unit.attrs.hp)) {
                unit.attrs.hp += item.amount;
                unit.items.shift();
                recoveringCandidates.push({
    ...
    this._recoveringCandidates = recoveringCandidates;
};

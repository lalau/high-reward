    ...
    members.forEach(function(member, memberIndex) {
        ...
        if (member) {
            if (!rowConfig || rowConfig.type !== 'member') {
                this._rows[count] = {
                    type: 'member',
                    row: this._renderMemberRow(x, y, member, memberIndex)
                };
            } else {
...
TroopMembersPanel.prototype._renderMemberRow = function(x, y, member, memberIndex) {
    var memberRow = new MemberRow(this.game, x, y, member, memberIndex);
